...
namespace ComposerSkillBotDotNet.Controllers
{
    ...
    public class ProactiveController : ControllerBase
    {
        ...
        public ProactiveController(ConversationState conversationState, IBotFrameworkHttpAdapter adapter, ConcurrentDictionary<string, ContinuationParameters> continuationParametersStore, IBot bot)
        {
        ...
        public async Task<IActionResult> Get(string user)
        {
            ...
            try
            {
                async Task ContinuationBotCallback(ITurnContext context, CancellationToken cancellationToken)
                {
                    ...
                    await _bot.OnTurnAsync(context, cancellationToken).ConfigureAwait(false);
